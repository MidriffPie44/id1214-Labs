...
class Block:
  ...
  def moveR(self):
    ...
    if self.right is None:
      return  # If block is on the rightmost spot, terminate
    ...
    if self.right.top() is not None:              # If there is block(s) on the right
      while self.right.top().value < self.value:  # Move it all the way left first
        self.right.top().moveL()
    
    self.stack_on(self.right.top())  # now current block is free to move right
    self.moveR() # repeat the whole process until block is all the way right
  ...
  def moveL(self):
    ...
    if self.left.top() is not None:
      while self.left.top().value < self.value:
        self.left.top().moveR()
